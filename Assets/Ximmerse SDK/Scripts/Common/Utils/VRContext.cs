...
namespace Ximmerse.VR{
	...
	public class VRContext:MonoBehaviour {
		...
		#region Nested Types
		[System.Serializable]
		public class NodeTransformPair:UKeyValuePair<VRNode,Transform>{}

	#if UNITY_EDITOR
		[UnityEditor.CustomPropertyDrawer(typeof(NodeTransformPair))]
		public class NodeTransformPairDrawer:UKeyValuePairDrawer<VRNode,Transform>{}
	#endif
		...
		public static VRContext main {
			get {
				if(!s_MainCached) {
					...
					//
					Ximmerse.Log.i("VRContext","Initialization Info : family = \""+s_Main.vrDevice.family+"\", isPresent = "+isPresent+", refreshRate = "+refreshRate+" Hz.");
				}
				return s_Main;
			}
		}
		...
		public virtual Quaternion GetHmdRotation() {
			if(vrDevice==null) {
				return Quaternion.identity;
			}else {
				return vrDevice.GetRotation();
			}
		}
