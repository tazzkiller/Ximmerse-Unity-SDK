...
public class TrackerFrustum:MonoBehaviour{
	...
	[System.NonSerialized]protected int m_GroundAlpha,m_WallAlpha;
	...
	protected virtual void Start() {
		...
		if(autoCreatePlayArea) {
			...
			if(m_PlayArea==null) 
            {
                m_PlayArea = trackingSpaceAnchor.GetComponentInChildren<PlayAreaRenderer>(true);
                if (m_PlayArea == null)
                {
	...
	public virtual void CheckBounds() {
		...
		if(m_TrackerHandle==-1||
			XDevicePlugin.GetInt(m_TrackerHandle,XDevicePlugin.kField_ConnectionState,0)!=(int)DeviceConnectionState.Connected
		){
			return;
		}
		//
